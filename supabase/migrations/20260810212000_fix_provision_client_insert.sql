-- Migration: 20260810193000_fix_provision_event_draft_fields.sql
-- Forward-only fix: Update app_private.provision_application_atomic to include
-- required NOT-NULL schema columns draft_event_slug and draft_visibility on public.rsvp_events.
...
begin
  ...
  if v_client.id is null then
    insert into public.clients (
      name, contact_name, contact_email, contact_phone, status, plan_type, created_at, updated_at
    ) values (
      v_app.full_name, v_app.full_name, v_normalized_email, v_app.phone, 'active', v_app.preferred_plan, v_now, v_now
  ...
  if v_event.id is null then
    ...
    insert into public.rsvp_events (
      client_id, title, event_type, event_date, event_location, event_slug, draft_event_slug, draft_visibility, status, max_guest_count, created_at, updated_at
    ) values (
      v_client.id, v_title, v_app.event_type, v_app.event_date, v_app.event_location, v_slug, v_slug, 'private', 'draft', coalesce(v_app.estimated_guest_count, 100), v_now, v_now
