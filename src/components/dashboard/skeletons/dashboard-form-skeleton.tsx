    ...
    <DashboardCardSkeleton className="overflow-hidden rounded-[1.75rem]">
      <div className="space-y-4">
        <div className="space-y-3">
          ...
          <DashboardSkeletonLine className={compact ? "h-8 w-48 rounded-2xl" : "h-10 w-56 rounded-2xl"} />
        ...
        <div className={compact ? "space-y-3" : "space-y-4"}>
          {Array.from({ length: rows }).map((_, index) => (
            <div key={index} className="space-y-2">
              ...
              <DashboardSkeletonBlock className={compact ? "h-11 w-full rounded-xl" : "h-12 w-full rounded-2xl"} />
            </div>
          ))}
        </div>
