...
export function BillingPaymentInstructions({
  ...
  return (
    <div className="text-center">
      <p className="mx-auto max-w-[22rem] text-[15px] font-medium leading-7 text-[color:var(--dash-muted)]">
...
function PaymentOptionDialog({ option }: { option: BillingPaymentOption }) {
  ...
  return (
    <Dialog>
      ...
      <DialogContent
        className="max-w-[calc(100%-1.5rem)] gap-0 overflow-hidden rounded-[1.6rem] border border-[#ead8c4] bg-[#fffaf3] p-0 text-[color:var(--dash-foreground)] shadow-[0_24px_60px_rgba(83,54,33,0.16)] ring-[#f1e2d0]/80 sm:max-w-[540px]"
      >
        <div className="px-5 pt-5 pb-0 sm:px-6 sm:pt-6">
          ...
          <div className="mt-5 grid gap-4 pb-5">
            <DetailBlock label="Account name" value={option.accountName || "—"} />
            <div className="rounded-[1.2rem] border border-[#efe0cf] bg-[color:color-mix(in_srgb,#fff9f1_78%,white)] px-4 py-3.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)]">
              <div className="flex items-start justify-between gap-3">
                ...
                {option.accountNumber ? (
                  <Button
                    ...
                    className="shrink-0 rounded-full border-[#e6cfbb] bg-[#fff7ee] px-3.5 text-[color:var(--dash-brand)] shadow-[inset_0_1px_0_rgba(255,255,255,0.82)] hover:border-[#ddb493] hover:bg-[#fff1e5] hover:text-[color:var(--dash-brand)]"
                  >
            ...
            <div className="rounded-[1.25rem] border border-[#efe0cf] bg-[color:color-mix(in_srgb,#fff8ef_76%,white)] px-4 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)]">
              <p className="text-center text-xs font-bold uppercase tracking-[0.14em] text-[color:var(--dash-heading-muted)]">
                QR code
              </p>
              {option.qrImageUrl ? (
                <div className="mt-3 flex justify-center rounded-[1.05rem] border border-[#ebdccc] bg-white px-3 py-3">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={option.qrImageUrl}
                    alt={`${providerLabel} payment QR code`}
                    className="mx-auto aspect-square w-full max-w-[340px] max-h-[40dvh] object-contain"
                  />
                </div>
              ) : (
                <div className="mt-3 flex min-h-36 items-center justify-center rounded-[1rem] border border-dashed border-[#e6d2be] bg-white/65 px-4 text-center text-sm font-medium text-[color:var(--dash-heading-muted)]">
        ...
        <DialogFooter
          showCloseButton
          className="border-[#ecd9c6] bg-[color:color-mix(in_srgb,#fdf3e8_72%,white)] px-5 py-4 sm:px-6"
        />
      </DialogContent>
    </Dialog>
  );
